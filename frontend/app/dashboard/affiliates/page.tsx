...
export default function AffiliatesPage() {
  ...
  return (
    <div className="w-full">
      <div className="mb-8">
        <h1 className="text-2xl font-bold mb-2 text-white">Affiliate Program</h1>
        <p className="text-slate-400">Earn 20% commission on referral trading fees</p>
        ...
        {stats?.referralCode ? (
          <div className="flex flex-col sm:flex-row gap-3">
            <input
              ...
              className="flex-1 h-11 px-4 rounded-lg bg-slate-800/80 border border-slate-700 text-sm font-mono text-white focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20"
            />
            <Button
              onClick={copyToClipboard}
              variant={copied ? 'outline' : 'default'}
              className={`shrink-0 ${copied ? 'border-teal-500/30 text-teal-400' : 'bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-600 hover:to-cyan-600 text-white border-0 shadow-lg shadow-teal-500/20'}`}
            >
          ...
          <Button
            ...
            disabled={generating}
            className="w-full bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-600 hover:to-cyan-600 text-white border-0 shadow-lg shadow-teal-500/20 disabled:opacity-50"
          >
      ...
      <div className="rounded-2xl border border-slate-700/50 bg-slate-900/50 p-6">
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={item.step} className="flex gap-4 items-start p-3 rounded-xl hover:bg-teal-500/5 transition-colors">
              <div className="w-10 h-10 rounded-xl bg-teal-500/15 flex items-center justify-center shrink-0 border border-teal-500/30">
                <span className="text-sm font-bold text-teal-400">{item.step}</span>
              </div>
              <div>
...
function StatCard({ icon, label, value, accent }: { icon: React.ReactNode; label: string; value: string; accent?: boolean }) {
  return (
    <div className="rounded-2xl border border-slate-700/50 bg-slate-900/50 p-5 hover:border-teal-500/30 hover:shadow-[0_0_24px_-8px_rgba(20,184,166,0.12)] transition-all duration-200">
      <div className="w-10 h-10 rounded-xl bg-teal-500/15 flex items-center justify-center text-teal-400 mb-3 border border-teal-500/30">
        {icon}
      </div>
