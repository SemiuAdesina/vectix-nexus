  ...
  if (!state) {
    return (
      <div className="glass rounded-xl p-6">
        <p className="text-muted-foreground">Circuit breaker not initialized</p>
      </div>
    );
  }
    ...
    <div className="glass rounded-xl p-6">
      ...
      <div className="space-y-3 mb-4">
        <div className="flex justify-between text-sm">
          ...
          <span>{state.failureCount}</span>
        </div>
        {state.pausedUntil && (
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">Paused Until</span>
            <span>{new Date(state.pausedUntil).toLocaleString()}</span>
          </div>
        )}
        <div className="flex justify-between text-sm">
          <span className="text-muted-foreground">Last Reset</span>
          <span>{new Date(state.lastResetTime).toLocaleString()}</span>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        <button
          onClick={handleCheck}
          disabled={loading}
          className="flex-1 px-4 py-2 bg-secondary text-secondary-foreground rounded-lg hover:bg-secondary/80 disabled:opacity-50"
        >
          Check
        </button>
        {state.status === 'open' && (
