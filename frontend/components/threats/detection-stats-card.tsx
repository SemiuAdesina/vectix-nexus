  ...
  return (
    <div className="rounded-2xl border border-slate-700/50 bg-slate-900/50 p-6 shadow-[0_0_24px_-8px_rgba(20,184,166,0.08)]">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h2 className="text-lg font-semibold text-white">Detection Stats</h2>
          <p className="text-sm text-slate-400">Threat intelligence metrics by severity</p>
        </div>
      </div>
      <div className="space-y-3">
        {stats.map((stat) => {
          ...
          return (
            <div key={stat.label} className="flex items-center justify-between p-3 rounded-xl border border-slate-700/50 bg-slate-800/50 hover:border-teal-500/40 transition-colors">
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 border ${stat.box}`}>
