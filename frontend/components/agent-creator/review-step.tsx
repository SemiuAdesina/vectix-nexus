    ...
    <div className="space-y-6">
      <div>
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h3 className="text-base sm:text-lg font-semibold text-white">Review & Deploy</h3>
            <p className="text-xs sm:text-sm text-slate-400">Verify your configuration before deployment</p>
          </div>
        </div>
      ...
      <div className={CARD}>
        <div className="space-y-2">
          {checks.map((c) => (
            <div key={c.label} className="flex items-center justify-between p-2.5 sm:p-3 rounded-lg sm:rounded-xl border border-slate-700/50 bg-slate-800/50 hover:border-teal-500/30 transition-colors">
              <span className="text-xs sm:text-sm font-medium text-white">{c.label}</span>
              <span className={`text-[10px] sm:text-xs px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-md sm:rounded-lg flex items-center gap-1 sm:gap-1.5 font-medium ${c.ok ? 'bg-teal-500/15 text-teal-400 border border-teal-500/30' : 'bg-slate-800 text-slate-500 border border-slate-600'}`}>
      ...
      <div className={CARD}>
        ...
        <div className="rounded-lg sm:rounded-xl border border-slate-700/50 bg-slate-950/80 p-3 sm:p-4 max-h-[160px] sm:max-h-[200px] overflow-auto">
          <pre className="text-[10px] sm:text-xs font-mono text-slate-300 whitespace-pre-wrap break-all sm:break-words">{generatedJson}</pre>
        </div>
      </div>
      ...
      <div className="flex gap-2 sm:gap-3 pt-2">
        ...
        <Button size="default" onClick={onDeploy} disabled={isDeploying} className="flex-1 bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-600 hover:to-cyan-600 text-white border-0 shadow-lg shadow-teal-500/20 disabled:opacity-50 text-sm sm:text-base">
          {isDeploying ? <><Loader2 className="w-4 h-4 animate-spin mr-1 sm:mr-2" /> Deploying...</> : <><Rocket className="w-4 h-4 mr-1 sm:mr-2" /> Deploy Agent</>}
        </Button>
      </div>

      {deployResult && (
        <div className={`rounded-xl sm:rounded-2xl border p-4 sm:p-5 ${deployResult.success ? 'bg-teal-500/10 border-teal-500/30' : 'bg-red-500/10 border-red-500/30'}`}>
